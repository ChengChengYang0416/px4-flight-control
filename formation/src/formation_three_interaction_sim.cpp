...
geometry_msgs::PoseStamped uav2;
	...
	if(link_states.name.size()>0) {
		for(unsigned int i=0; i<link_states.name.size(); i++) {
			if(link_states.name[i].compare("iris_0::base_link")==0) {
				...
				ROS_INFO("uav0 x = %f", uav0.pose.position.x);
				ROS_INFO("uav0 y = %f", uav0.pose.position.y);
				ROS_INFO("uav0 z = %f", uav0.pose.position.z);
			}
			...
			if(link_states.name[i].compare("iris_1::base_link")==0) {
				...
				ROS_INFO("uav1 x = %f", uav1.pose.position.x);
				ROS_INFO("uav1 y = %f", uav1.pose.position.y);
				ROS_INFO("uav1 z = %f", uav1.pose.position.z);
			}

			if(link_states.name[i].compare("iris_2::base_link")==0) {
				uav2.pose.position.x = link_states.pose[i].position.x;
				uav2.pose.position.y = link_states.pose[i].position.y;
				uav2.pose.position.z = link_states.pose[i].position.z;
				ROS_INFO("uav2 x = %f", uav2.pose.position.x);
				ROS_INFO("uav2 y = %f", uav2.pose.position.y);
				ROS_INFO("uav2 z = %f", uav2.pose.position.z);
			}
		}
	}
}
...
int main(int argc, char **argv)
{
	...
	ros::Subscriber state_sub3 = nh.subscribe<mavros_msgs::State>
	                             ("uac2/mavros/state", 10, state_cb3);
	...
	ros::Publisher local_vel_pub3 = nh.advertise<geometry_msgs::TwistStamped>("uav2/mavros/setpoint_velocity/cmd_vel", 1);
	...
	while (ros::ok()) {
		...
		ros::spinOnce();
		follow(vir1,uav0,&vs,dis1,uav1,dis2,uav2,dis3);        //be careful of the sign
		//mocap_pos_pub.publish(host_mocap);
		local_vel_pub.publish(vs);
		ros::spinOnce();
		follow(vir1,uav1,&vs2,dis2,uav0,dis1,uav2,dis3);     //be careful of the sign
		//mocap_pos_pub2.publish(host_mocap2);
		local_vel_pub2.publish(vs2);
		ros::spinOnce();
		follow(vir1,uav2,&vs3,dis3,uav0,dis1,uav2,dis2);     //be careful of the sign
		//mocap_pos_pub3.publish(host_mocap3);
		local_vel_pub3.publish(vs3);
